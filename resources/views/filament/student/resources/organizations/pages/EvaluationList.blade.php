				...
				<x-filament::card>
					...
					<div class="mt-4">
						@if($task['task_type'] === 'Self-Evaluation')
							<x-filament::button 
								tag="a"
								:href="route('filament.student.resources.organizations.self-evaluate', ['organization' => $task['organization_id']])"
								size="sm"
						...
						@else
							<x-filament::button 
								tag="a"
								:href="route('filament.student.resources.organizations.peer-evaluate', ['organization' => $task['organization_id'], 'student' => $task['target_id']])"
								size="sm"
								:icon="$task['status'] === 'Completed' ? 'heroicon-s-eye' : 'heroicon-s-pencil-square'">
								@if($task['status'] === 'Completed')
									View Evaluation
