...
class StudentsRelationManager extends RelationManager
{
    ...
    protected function getTableActions(): array
    {
        ...
        // Only show Assign Peer Evaluatees in edit org page
        if ($this->pageClass === \App\Filament\Admin\Resources\Organizations\Pages\EditOrganization::class) {
            array_unshift($actions, $this->createPeerAssignmentAction());
        }
        // In view org page, do not show the Actions dropdown (ActionGroup)
        // Only show Evaluate button and not the ActionGroup
        if ($this->pageClass === \App\Filament\Admin\Resources\Organizations\Pages\ViewOrganization::class) {
                ...
                \Filament\Forms\Components\CheckboxList::make('peer_evaluatees')
                    ...
                    ->options(function ($record) {
                        ...
                        $allStudentIds = $this->ownerRecord->students()
                            ...
                            ->pluck('students.id')
                            ->toArray();
                        $alreadyAssignedIds = OrganizationPeerEvaluator::where('organization_id', $this->ownerRecord->id)
                        ...
                        $currentAssignedIds = OrganizationPeerEvaluator::where('organization_id', $this->ownerRecord->id)
                            ->where('evaluator_student_id', $record->id)
                            ->pluck('evaluatee_student_id')
                            ->toArray();
    ...
    protected function getDetachAction(): DetachAction
    {
        return DetachAction::make()
            ...
            ->after(function ($record) {
                // Remove all peer evaluator assignments for this student in this organization
